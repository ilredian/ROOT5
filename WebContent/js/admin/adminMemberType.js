...
console.log(123);
	...
	$hide.hide();
	$tr.empty();
	$tr.append("<td colspan='3'><form action='updateMemberType.go' method='post' onsubmit='return updateOK()'><input type='text' name='tno' value='"+td1+"' readonly><input type='text' name='tn' value='"+td2+"'><input class='btn btn-primary btn-sm' type='submit' value='수정 완료'><input type='button' class='btn btn-danger btn-sm' value='수정 취소' onclick='updateCancel()'></form></td>");

});

function updateCancel(){
	location.replace('adminMemberType.go');
}
