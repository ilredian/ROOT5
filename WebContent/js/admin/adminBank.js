$('.update').click(function(data){
	...
	$hide.hide();
	$tr.empty();
	$tr.append("<td colspan='2'><form action='updateBank.go' method='post' onsubmit='return updateOK()'><input type='text' name='bank' value='"+td1+"'><input type='hidden' name='origin' value='"+td1+"'><input class='btn btn-primary btn-sm' type='submit' value='수정 완료'><input type='button' class='btn btn-danger btn-sm' value='수정 취소' onclick='updateCancel()'></form></td>");

});

function updateCancel(){
	location.replace('adminBank.go');
}
