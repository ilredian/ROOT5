<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<ul>
	<il class="menu"><a><input type="submit" class="btn btn-danger btn-sm" value="신고관리"></a>
		<ul class="hide">
			<il><a href="adminBoardFree.go?cno=1">ㄴ자유게시판</a></il>
			<il><a href="adminBoardPhoto.go?cno=3">ㄴ사진게시판</a></il>
			<il><a href="adminCommment.go">ㄴ댓글</a></il>
		</ul></il>
	<il class="menu"><a><input type="submit" class="btn btn-danger btn-sm" value="메뉴관리"></a>
		<ul class="hide">
			<il><a href="adminDomainList.go">ㄴ사이트명</a></il><br>
			<il><a href="adminItemList.go">ㄴ거래물품</a></il><br>
			<il><a href="adminBank.go">ㄴ은행종류</a></il><br>
			<il><a href="adminMemberType.go">ㄴ회원종류</a></il><br>
			<il><a href="adminStatementCategory.go">ㄴ사기분류</a></il><br>
			<il><a href="adminMember.go">ㄴ회원관리</a></il><br>
			<il><a href="adminBoardCategory.go">ㄴ카테고리</a></il><br>
		</ul></il>
	<il class="menu"><a><input type="submit" class="btn btn-danger btn-sm" value="메일함&nbsp;&nbsp;&nbsp;&nbsp;"></a>
		<ul class="hide">
			<il><a href="adminMail.go">ㄴ메일 보내기</a></il>
			<il><a href="adminMailRE.go">ㄴ받은 메일</a></il>
		</ul></il>
</ul>